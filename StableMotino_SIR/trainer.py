import os
server_id = os.environ.get("SERVER_ID_LOCAL")
if server_id == "33":
...
def log_validation(vae,text_encoder,tokenizer,unet,args,accelerator,weight_dtype,scheduler,epoch,
                   input_image_path="/home/suilin02/DIR-D/testing/input/00001.jpg",
                   input_mask_path="/home/suilin02/DIR-D/testing/mask/00001.jpg"
                   ):
    # TODO: denoise_steps
    denoise_steps = 1
    batch_size = 1
    ...
    with torch.no_grad():
        ...
        input_mask_pil = Image.open(input_mask_path)
...
def main():    
    ...
    noise_scheduler = DDIMScheduler.from_pretrained(args.pretrained_model_name_or_path,subfolder='scheduler',timestep_spacing='trailing',local_files_only=True)
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        if SD_flag:
            ...
            unet = UNet2DConditionModel.from_pretrained(args.pretrained_model_name_or_path,subfolder="unet",
                                                        in_channels=4, sample_size=(48, 64),
            ...
            with torch.no_grad():
                ...
                new_conv_in.weight[:, 4:8, :, :].copy_(unet.conv_in.weight)
                new_conv_in.weight[:, 8:, :, :].copy_(unet.conv_in.weight)
                new_conv_in.weight[:, :, :, :] = new_conv_in.weight[:, :, :, :] / 3.0
        ...
        if not SD_flag:
            ...
            unet = UNet2DConditionModel.from_config(unet_config, in_channels=12, sample_size=(48, 64))
    ...
    if args.use_ema:
        if SD_flag:
            ...
            with torch.no_grad():
                ...
                new_conv_in.weight[:, 8:, :, :].copy_(ema_unet.conv_in.weight)
                new_conv_in.weight[:, :, :, :] = new_conv_in.weight[:, :, :, :] / 3.0
        ...
        if not SD_flag:
            # unet_config = UNet2DConditionModel.load_config(args.pretrained_model_name_or_path, subfolder="unet")
            unet_config = UNet2DConditionModel.load_config('./unet_config.json')
            ema_unet = UNet2DConditionModel.from_config(unet_config, in_channels=12, sample_size=(48, 64))
            ema_unet = EMAModel(ema_unet.parameters(), model_cls=UNet2DConditionModel, model_config=ema_unet.config)
            ema_unet.to(accelerator.device)
    ...
    with accelerator.main_process_first():
        (train_loader, test_loader), dataset_config_dict = prepare_dataset(
            flow_datapath=args.flow_datapath,
            condition_datapath = args.condition_datapath,
            mask_datapath = args.mask_datapath,
            gt_datapath = args.gt_datapath,
            train_condition_list = args.train_condition_list, 
            train_flow_list = args.train_flow_list,
            train_mask_list = args.train_mask_list,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                memory_show("first vae")

                if partial_fp16:
                    h_mask = vae.encoder(mask.half())
                    ...
                    mask_latents = mask_latents.to(weight_dtype)
                else:
                    h_mask = vae.encoder(mask.to(weight_dtype))
                    moments_mask = vae.quant_conv(h_mask)
                    mean_mask, logvar_mask = torch.chunk(moments_mask, 2, dim=1)
                    mask_latents = mean_mask * mask_latent_scale_factor

                memory_show("second vae")
                ...
                unet_input = torch.cat([mask_latents, rgb_latents, noisy_disp_latents], dim=1)  # this order is important: [1,8,H,W]
                ...
                flow_pred = flow * 50 # vital factor
                flow_pred = flow_pred[:, :2, :, :]

                pred_sample = flow_warp(cond, flow_pred)
                pred_mask = flow_warp(mask, flow_pred)
                pred_mask[pred_mask > 0.5] = 1
                full_mask = torch.ones_like(mask)

                loss_pl = F.mse_loss(gt, pred_sample, reduction='mean')
                loss_mask = F.mse_loss(full_mask, pred_mask, reduction='mean')
                loss_vgg = crit_vgg(pred_sample, gt, target_is_features=False)

                if args.normalized_input:
                    loss = loss_latentNoise + loss_pl * 8 + loss_mask * 8 + loss_vgg * 0.1
                else:
                    loss = loss_latentNoise + loss_pl * 32 + loss_mask * 32 + loss_vgg * 0.4
                ...
                avg_loss_mask = accelerator.gather(loss_mask.repeat(args.train_batch_size)).mean()
                train_loss_mask = avg_loss_mask.item() / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log({"train_loss_mask": train_loss_mask}, step=global_step)

...
class SDRecPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        # --------------- Image Processing ------------------------
        image = self.transform(input_image).expand(3, -1, -1)
        mask = self.transform(mask).expand(3, -1, -1)
        ...
        # mask = mask.to(self.weight_dtype)
        mask = mask.to(device)
        if self.partial_fp16:
            mask = mask.half()
        else:
        ...
        duplicated_mask = torch.stack([mask])
        single_mask_dataset = TensorDataset(duplicated_mask)
        ...
        single_mask_loader = DataLoader(single_mask_dataset, batch_size=_bs, shuffle=False)
        
        # predicted the flow
        flow_pred_ls = []
        
        for rgb, mask in zip(single_rgb_loader, single_mask_loader):
            (batched_rgb,) = rgb  # here the image is still around 0-1
            (batched_mask, ) = mask
            flow_pred_raw = self.single_infer(
                input_rgb=batched_rgb,
                input_mask=batched_mask, 
        ...
        warped_img = flow_warp(rgb_norm.unsqueeze(0), flow_pred.unsqueeze(0)).squeeze(0)
    ...
    def single_infer(self,
        ...
        mask_latent = self.encode_RGB(input_mask)
        ...
        for i, t in iterable:
            unet_input = torch.cat(
                [mask_latent, rgb_latent, flow_latent], dim=1
